	...
	if ( isCommCapable ) {
		...
		if ( dataFile ) {
			...
			if ( buffer ) {
         	...
         	// FLStatus flWriteChannel(struct FLContext *handle,uint32 timeout, uint8 chan, uint32 count, const uint8 *data, const char ** error);
         	// Returns FL_SUCCESS/FL_PROTOCOL_ERR/FL_USB_ERR
		      //                      handle, timeout, channel,   count,   data,  error
				status = flWriteChannel(handle,   30000,    0x00, fileLen, buffer, &error);
				CHECK(22);
            status = flWriteChannel(handle,   1000,    0x01, 1, buf1, &error);
            status = flWriteChannel(handle,   1000,    0x02, 1, buf2, &error);
			} else {
	   ...
	   // FLStatus flReadChannel(struct FLContext *handle, uint32 timeout, uint8 chan, uint32 count, uint8 *buf, const char **error)
      //                     handle, timeout, channel, count, buf,  error 		
		status = flReadChannel(handle,    1000,    0x00,     2, buf, &error);
		CHECK(24);
		printf("Got 0x%02X\n", buf[0]);
		printf("Reading channel...\n");
		status = flReadChannel(handle, 1000, 0x01, 2, buf, &error);
		CHECK(25);
		printf("Got 0x%02X\n", buf[0]);
		printf("Reading channel...\n");
		status = flReadChannel(handle, 1000, 0x02, 2, buf, &error);
		CHECK(26);
		printf("Got 0x%02X\n", buf[0]);
		printf("Reading channel...\n");
                status = flReadChannel(handle, 1000, 0x01, 2, buf, &error);
