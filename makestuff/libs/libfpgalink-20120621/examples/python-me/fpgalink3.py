...
if __name__ == "__main__":
    ...
    try:
        ...
        if ( isCommCapable ):
            ...
            count = 0
            while count < 5000:
                buff = flReadChannel(handle, 1000, 0x00, 50)
                count += 50
                end = 50 - 1
                ndx = 0
                while ndx < end:
            ...
            print("Start")
            t0 = time.clock()
            buff = flReadChannel(handle, 10000, 0x00, 614400)
            t1 = time.clock()
            for i in range(640):
                for j in range(480):
                  im.putpixel((i, j), ((buff[ndx] & 0xf8), ((buff[ndx] & 0x07) << 5) | ((buff[ndx+1] & 0xe0) >> 3), (buff[ndx+1] & 0x1f) << 3))
                  #buf0 = flReadChannel(handle, 1000, 0x02, 1)
                  #buf1 = flReadChannel(handle, 1000, 0x03, 1)
                  #im.putpixel((i, j), ((buf0 & 0xf8), ((buf0 & 0x07) << 5) | ((buf1 & 0xe0) >> 3), (buf1 & 0x1f) << 3))
                  #im.putpixel((i,j), (0, buff[ndx], 0))
                  ndx += 2
            t2 = time.clock()

            #print("x%02X x%02X x%02X x%02X x%02X" % (buff[0], buff[1], buff[2], buff[3], buff[4]))
            print("t0 = %f" % t0)
            print("t1 = %f" % t1)
            print("t2 = %f" % t2)
            
            for i in range(500):
               print("buff[%d] = x%02X" % (i, buff[i]))
