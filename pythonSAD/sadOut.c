...
int main(int argc, char** argv)
{
   ...
   int arrSad[atoi(argv[3])-(atoi(argv[5])-1)][atoi(argv[4])-(atoi(argv[6]) + atoi(argv[5])-2)];
   ...
   if (fscanf(fpR, "%d", &r) == 1 && fscanf(fpL, "%d", &l) == 1)
   {
      arrR[0][0] = r;
      arrL[0][0] = l;
   }
   ...
   {
      printf("Error reading integers (0,0).\n");
   }
      
   i = 0;
   j = 1;
   while (i < height && j < width && !feof(fpR) && !feof(fpL))
   {
      //printf("%d ", l);
      arrR[i][j] = r;
      arrL[i][j] = l;
      ...
      {
         printf("Error reading integers (%d, %d).\n", i, j);
      }
   }
   ...
   // Write sad values to output file
   fpSad = fopen("cSadVals_7x7.txt", "w");
   for (i = 0; i < dispH; i++)
   {
      for (j = 0; j < dispW; j++)
      {
         fprintf(fpDisp, "%d ", arrDisp[i][j]);
         fprintf(fpSad, "%d ", arrSad[i][j]);
      }
      fprintf(fpDisp, "\n");
      fprintf(fpSad, "\n");
   }
